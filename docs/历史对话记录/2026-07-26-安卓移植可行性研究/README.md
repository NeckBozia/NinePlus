# 2026-07-26-安卓移植可行性研究

这个 Android 移植项目的全部规划、调研和 iOS 侧改动都出自一次会话。这里是那次会话的**原始记录**，不是整理稿 —— `docs/移植可行性研究/` 下的文档是从它产出的结论，这里是产出过程。目录结构的说明见[上一层的 README](../README.md)。
...
结论已经写在 `docs/移植可行性研究/` 下的文档里了 —— 那些是给人看的。这份留的是**过程**：某个数字是怎么查出来的、某条结论推翻了什么、哪些路走过发现是死的。
...
| 目录 | 内容 |
| --- | --- |
| `docs/移植可行性研究/` | 全部规划、规格与调研 |
| `docs/历史对话记录/` | 本目录，会话原始记录 |
| `docs/当前状态.md` | 交接文档，新会话从这里读起 |
