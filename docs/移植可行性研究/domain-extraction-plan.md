...
五步全部做完，每步单独提交，CI 逐步验证。

| 步 | 产出 | 状态 |
| --- | --- | --- |
| 1 | `Shared/NinebotTripTrend.swift` —— 分析引擎搬出界面，5 个规则阈值提成有名字的常量，规则返回 `NinebotTripInsight` 枚举，中文留在界面 | 完成 |
| 2 | `Shared/NinebotFormatting.swift` —— 21 个格式化与日期函数搬走；颜色和图标映射按约定留在界面 | 完成 |
| 3 | `Domain/NinebotRideRecorder.swift` —— 556 行采集器整体挪出，13 个阈值集中在一处 | 完成 |
| 4 | 模型层孪生属性，见下 | 完成（三项待你定，见 pending-decisions 的 D10） |
| 5 | `Domain/NinebotLoadingOperation.swift` —— 下拉刷新指示器不再靠匹配中文判断 | 完成 |

第四步的口径是**只抽含判断分支的属性**（纯格式化的那些抽了没意义），筛出 14 个：

| 抽成 | 覆盖 | 附带发现 |
| --- | --- | --- |
| `Shared/NinebotChargingStatus.swift`（4 个枚举） | 7 个属性：上电状态、充电状态、充电摘要、到 80% 与充满的时间与时刻 | D9：服务端充满时刻和「现在」比对而不是和快照时间比对 |
| `Shared/NinebotRangeEstimate.swift`（3 个枚举 + 4 个服务端契约常量） | 3 个属性：预测质量、精度来源、估算依据 | 置信阈值 0.82 之前是裸数字 |
| `Shared/NinebotHistoryPeriod.swift`（1 个枚举） | 1 个属性：记录周期 | 边界取整怪相：3599 秒仍在分钟档但显示「60 分钟」 |
| 未抽 | 3 个属性 | D10，交给你定 |

**现状**：模型层剩 43 个产出中文的属性，其中只有 D10 那三个还带内联分支，其余全是「有值就格式化、没值就兜底」的单条通路。测试从 291 增到 **376**，Dashboard 从 5420 行降到 5215 行。

**没做的**：共享 JSON 测试夹具（原计划约 1 天）。抽取出来的枚举现在只有 Swift 侧测试。Android 开工时补，那时候才知道两端要对齐哪些输入输出。
